...
/**
 * Simple graph library for directed graphs with just one edge, adapted from DirectedMultiGraph.
 *
...
public class DirectedGraph<V, E> implements Graph<V, E> /* Serializable */{
  ...
  Map<V, Map<V, E>> outgoingEdges = Generics.newHashMap();

  Map<V, Map<V, E>> incomingEdges = Generics.newHashMap();

  public DirectedGraph() {
  }

  public DirectedGraph(DirectedGraph<V,E> graph) {
  ...
  public boolean equals(Object that) {
    if (that == this)
      return true;
    if (!(that instanceof DirectedGraph))
      return false;
    return outgoingEdges.equals(((DirectedGraph) that).outgoingEdges);
  }
  ...
  public boolean addVertex(V v) {
    if (outgoingEdges.containsKey(v))
      return false;
    outgoingEdges.put(v, Generics.<V, E>newHashMap());
    incomingEdges.put(v, Generics.<V, E>newHashMap());
    return true;
  }
  ...
  public void add(V source, V dest, E data) {
    ...
    outgoingEdges.get(source).put(dest, data);
    incomingEdges.get(dest).put(source, data);
  }
  ...
  public boolean removeEdge(V source, V dest, E data) {
    if( outgoingEdges.containsKey(source) &&
        outgoingEdges.get(source).containsKey(dest) &&
        outgoingEdges.get(source).get(dest).equals(data) ) {
      removeEdges(source, dest);
      return true;
    }
    return false;
  }
  ...
  public List<E> getOutgoingEdges(V v) {
    return CollectionUtils.toList(outgoingEdges.get(v).values());
  }

  public List<E> getIncomingEdges(V v) {
    return CollectionUtils.toList(incomingEdges.get(v).values());
  }

  public int getNumEdges() {
    int count = 0;
    for (Map.Entry<V, Map<V, E>> sourceEntry : outgoingEdges.entrySet()) {
      count += sourceEntry.getValue().size();
    }
    return count;
  }

  public Set<V> getParents(V vertex) {
    Map<V, E> parentMap = incomingEdges.get(vertex);
    if (parentMap == null)
      return null;
  ...
  public Set<V> getChildren(V vertex) {
    Map<V, E> childMap = outgoingEdges.get(vertex);
    if (childMap == null)
      return null;
  ...
  public boolean isEdge(V source, V dest) {
    Map<V, E> childrenMap = outgoingEdges.get(source);
    if (childrenMap == null || childrenMap.isEmpty())
      return false;
    return childrenMap.containsKey(dest);
  }
  ...
  public List<E> getAllEdges() {
    ...
    for (Map<V, E> e : outgoingEdges.values()) {
  ...
  public List<E> getEdges(V source, V dest) {
    ...
    if( childrenMap.containsKey(dest) ) {
      return Collections.singletonList(childrenMap.get(dest));
    } else
      return Collections.emptyList();
  }
  ...
  public Iterable<E> edgeIterable() {
    return new Iterable<E>() {
      public Iterator<E> iterator() {
        return new EdgeIterator<V, E>(DirectedGraph.this);
      }
    };
  }

  static class EdgeIterator<V, E> implements Iterator<E> {
    private Iterator<Map<V, E>> vertexIterator;
    private Iterator<E> edgeIterator;

    E next;

    public EdgeIterator(DirectedGraph<V, E> graph) {
      vertexIterator = graph.outgoingEdges.values().iterator();
      primeNext();
    }

    public EdgeIterator(Map<V, Map<V, E>> source, V startVertex) {
      Map<V, E> children = source.get(startVertex);
      if (children == null) {
        return;
      }
      edgeIterator = children.values().iterator();
      primeNext();
    }
    ...
    private void primeNext() {
      while (true) {
        ...
        if (vertexIterator != null && vertexIterator.hasNext()) {
          edgeIterator = vertexIterator.next().values().iterator();
          continue;
        }
  ...
  public String toString() {
    ...
    for (V source : outgoingEdges.keySet()) {
      for (Map.Entry<V,E> dest : outgoingEdges.get(source).entrySet()) {
        s.append("  ").append(source).append(" -> ").append(dest.getKey()).append(" : ").append(dest.getValue()).append('\n');
      }
    }
    s.append('}');
    return s.toString();
