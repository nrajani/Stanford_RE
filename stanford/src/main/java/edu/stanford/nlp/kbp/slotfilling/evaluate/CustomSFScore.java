...
/**
 * This scorer is in theory identical to SFScore, but with minor tweaks to integrate in our system.
 */
...
//    }
    ...
    while ((line = responseReader.readLine()) != null) {
      assert !line.toLowerCase().contains("slash");
  ...
  public static Pair<Double, Double> scoreByRelationName(
      ...
      Maybe<Set<String>> scoreOnlyTheseValuesMaybe,
      Set<String> allQueryIds) throws IOException {
	//  System.out.println("abbd "+query + ":" + doc_id + ":" + answer_string);
      System.out.println("abbd "+keyFile);
    // reset
    judgement = new HashMap<String, Integer> ();
    equivalenceClass = new HashMap<String, Integer> ();
    query_has_answer = new HashMap<String, Boolean> ();
...
//    }
    ...
    if(allQueryIds != null) {
      os.println("Computing true recall for " + allQueryIds.size() + " queries: " + allQueryIds);
      os.println("Number of single valued relations: "+RelationType.singleValuedRelations.length);
      for(String qid: allQueryIds) {
        // single-valued slots
        for(RelationType relationName: RelationType.singleValuedRelations) {
          String key = qid + ":" + OfficialOutputWriter.officialRelationName(relationName);
    ...
    for (String query : svSlots) {
      ...
      if (doc_id.equals("NIL")) {
        ...
        Integer J = judgement.get(query + ":" + doc_id + ":" + answer_string);
        if (J == null) {
          os.println ("No judgement for xxx " +
              query + ":" + doc_id + " " + answer_string);
          J = WRONG;
        }
        int j = J;
